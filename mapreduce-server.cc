...
/**
 * Method: stageMappedFiles
 * ------------------
 * Examines the named directory for its legit entries and populates the provided
 * list with file name patterns instead of actual file names.
 */
void MapReduceServer::stageMappedFiles(const string& directory, list<string>& files) throw() {
  DIR *dir = opendir(directory.c_str());
  if (dir == NULL) {
    cerr << "Directory named \"" << directory << "\" could not be opened." << endl;
    exit(1); 
  } 
  size_t limit = numMappers * numReducers;
  for (size_t i=0; i<limit; i++){
    string file(directory);
    file = file + "/" + numberToString(i) + ".mapped";
    files.push_back(file);
  }
  closedir(dir); 
}
...
void MapReduceServer::spawnMappers() throw() {
  ...
  const size_t numHashCodes = numMappers*numReducers;
  size_t numServers = mapperNodes.size();
  for(size_t i=0; i<numMappers; i++){
  	string& mapperNode = mapperNodes[i % numServers];
  	string command = buildMapperCommand(mapperNode, mapperExecutable, intermediatePath, numHashCodes);
...
void MapReduceServer::spawnReducers() throw() {
  stageMappedFiles(intermediatePath, unprocessed);
  vector<string> reducerNodes(nodes);
  random_shuffle(reducerNodes.begin(), reducerNodes.end());
  size_t numServers = reducerNodes.size();
  for(size_t i=0; i<numReducers; i++){
        string& reducerNode = reducerNodes[i % numServers];
        string command = buildReducerCommand(reducerNode, reducerExecutable, outputPath);
        reducers.push_back(thread([this,reducerNode,command]{spawnWorker(reducerNode, command);}));
  }
  for(thread& t : reducers) t.join();
  if (verbose) cout << "Reducing of all mapped files now complete." << endl;
  dumpFileHashes(outputPath);
}
/**
 * Method: buildReducerCommand
 * --------------------------
 * Constructs the command needed to invoke a reducer on a remote machine using
 * the system (see "man system") function.
 */
